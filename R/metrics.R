...
#'
#' The function takes a peak group of class peakObj as input and calculates the
...
#'
#' The function takes a peak group of class peakObj as input and calculates
...
calculate_transition_sum <- function(x, endogenous.label = "light", standard.label = "heavy"){
  endogenous.cols <- grepl(endogenous.label,
                           colnames(x$PeakGroup[[1]]$sig))

  standard.cols <- grepl(standard.label,
                         colnames(x$PeakGroup[[1]]$sig))

  peak.sig <- data.frame(rowSums(x$PeakGroup[[1]]$sig[, endogenous.cols]),
                         rowSums(x$PeakGroup[[1]]$sig[, standard.cols]))
  names(peak.sig) <- paste0('sum.0.', c(endogenous.label, standard.label))
  area <- c(sum(x$PeakGroup[[1]]$area[endogenous.cols]),
            sum(x$PeakGroup[[1]]$area[standard.cols]))
  names(area) <- paste0("sum.0.", c(endogenous.label,standard.label))

  obj <- list(
    list(time = x$PeakGroup[[1]]$time,#numeric
